    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (Prefs.getString("last_patch_note_ver", "") != ver) {
            val dialog = CustomDialog(this@MainActivity, "$ver 패치노트",
                    """
                        [변경사항]
                        1. 이제 템플릿을 격자보기로 한눈에 볼 수 있습니다.
                        2. 이제 템플릿을 길게 탭하면 세부설정을 건너뛰고 바로 입력됩니다.
                        3. 메디컬 스티커팩(무료)이 추가되었습니다.
                        4. 이제 위젯에서 이전달 다음달로 넘어갈 수 있습니다.
                        5. 이제 위젯에서 글씨를 흰색으로 표시할 수 있습니다.
                        
                        [버그수정]
                        1. 시간설정한 기록 복사했을때 시간 변경되는 버그가 수정되었습니다.
                        2. 반복된 미완료 기록이 계속 표시되는 버그가 수정되었습니다.
                        
                        [가이드]
                        1. 기록에 태그를 설정하고 일간 화면 목록에서 태그를 탭하시면, 해당 태그가 붙은 모든 기록을 볼 수 있습니다. 
                        2. 메뉴 > 설정 > 캘린더 스타일 설정으로 가시면 글씨크기 폰트 등 다양한 스타일 속성들을 변경할 수 있습니다.

                        * 일부 사용자들에게 일간화면이 안보이는 버그는 문제의 심각성을 인지하고 최대한 빨리 고칠 수 있도록 노력하고 있습니다. 불편을 드려 죄송합니다 :(
                    """.trimIndent(), null, R.drawable.info) { result, _, _ ->
            }
            //* 메뉴 > 우측상단 아이콘을 탭하시면 달의기록 인스타그램에 방문 할 수 있습니다. 인스타그램을 팔로우하고 다양한 정보를 얻고 소통해보세요.
...
//            RecordManager.save(list)
    ...
    fun clearCalendarHighlight() {
        calendarPager.clearHighlight()
    }
